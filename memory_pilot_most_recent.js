...
async function experimentInit() {
 ...
 // check the fieldnames in the dictionary - does this participant already exist?
      ...
      num_keys_in_dict=Object.keys(existing_participants).length
      console.log(num_keys_in_dict)
      
      // if not add a new fieldname to this dictionary
      if(!existing_participants.includes(expInfo['workID'])){
          psychoJS.shelf.setDictionaryFieldValue({key: ["participants_IDs"], fieldName: expInfo['workID'], fieldValue :num_keys_in_dict})
          subject_number=num_keys_in_dict;
...
*/
  ...
  left_arrow = new visual.ImageStim({
    ...
    ori : 0.0, pos : [(- 0.1), 0], size : [0.15, 0.1],
